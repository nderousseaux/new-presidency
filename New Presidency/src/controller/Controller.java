        ...
        for(int j=0; j<m.getColumnSize(); j++){
            ...
            if(columnAbreviation.charAt(0) == 'I'){
                ...
                if(influencer.getValue() < 50.0){
                    value -= m.getCell(0, j) * (((1-(influencer.getValue()/influencer.getMaxValue())))-0.5) * i.getValue();
                }
                if(influencer.getValue() > 50.0){
                    value += m.getCell(0, j) * (((influencer.getValue()/influencer.getMaxValue()))-0.5) * i.getValue();
                }
            }
        }
        return value;
